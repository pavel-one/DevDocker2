...
Чертёж генерируется скриптом и всегда соответствует модели:

```bash
cd 3d-model
python3 make_drawing.py          # -> drawing.svg
rsvg-convert -o drawing.png drawing.svg   # (опционально) растровый PNG
```
...
| Рейки | идут вдоль длины (550 мм), на всю ширину полки — 5 шт, горшки стоят на сплошных рейках |
| Просвет между рейками | **30 мм** (ровно) |

Все размеры в модели заданы параметрически — их легко изменить в начале файла
`shelf_rack.scad`. Параметр `slat_gap` задаёт точный просвет между рейками
полки (количество реек подбирается автоматически, рейки центрируются с равными
отступами от краёв). Параметр
...
| `drawing.svg` / `drawing.png` | Чертёж с размерами (виды спереди, сбоку, полка сверху) |
| `make_drawing.py` | Скрипт-генератор чертежа |
